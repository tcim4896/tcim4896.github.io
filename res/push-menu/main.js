...
menuStructure = [
 	{
	    ...
	    list: [
	      {
	        text: "Mangler",
	        event: {type:"open",path:"mangler"},
	        list:[
	        	{
	        		text:"Drum and Bass",
	        		event:{type:"route",path:"drum-and-bass"}
	        	}
	        ]
	      },
	    ],
...
function pushMenu(menuStructure){
	let r=o({id:"menu",class:"menu"});
	for(let menuItem of menuStructure){
		console.log(menuItem)
		if(menuItem.event.type=="open"){
			for(let listItem of menuItem.list){
				b(s("menu"),o({
					text:menuItem.text,
					id:0,
					class:"level"
